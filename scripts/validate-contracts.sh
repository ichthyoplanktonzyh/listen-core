...
for (const path of ["/v1/health", "/v1/media", "/v1/media/{media_id}/progress", "/v1/media/{media_id}/subtitles", "/v1/subtitles/{track_id}", "/v1/word-profiles", "/v1/word-profiles/batch", "/v1/word-observations", "/v1/vocabulary", "/v1/vocabulary/export", "/v1/vocabulary/import", "/v1/vocabulary/import-external", "/v1/word-profiles/{profile_id}/details", "/v1/word-profiles/{profile_id}/learning-content", "/v1/media/{media_id}/availability", "/v1/events", "/v1/dictionary", "/v1/sentences/{sentence_id}/diagnosis"]) {
  if (!openapi.includes(path + ":")) throw new Error(`OpenAPI missing ${path}`);
}
for (const operation of ["health()", "registerMedia(", "updateProgress(", "importSubtitle(", "readSubtitle(", "updateWordProfile(", "readWordProfiles(", "createWordObservation(", "listVocabulary(", "wordDetails(", "updateWordLearningContent(", "exportVocabulary(", "importVocabulary(", "importExternalVocabulary(", "updateMediaAvailability(", "dictionaryLookup(", "diagnoseSentence("]) {
