...
rules="$(curl -fsS "${auth[@]}" "$base/v1/pronunciation/rules")"
node -e 'const v=JSON.parse(process.argv[1]);if(v.rules.length<15||v.rules.length>25||v.rules.some(x=>!x.rule_id||!x.description||!x.condition||!x.example||!x.counterexample))process.exit(1)' "$rules"
