...
json_assert "$summary" 'v.created===2&&v.invalid===0' "import-external should create 2 entries"

book="$(api_curl "$base/v1/vocabulary?language=en&status=known_recognized")"
profile_id="$(json_get "$book" '[0].profile.id')"
json_assert "$book" 'v.length===1' "should have 1 known_recognized entry"

details="$(api_curl -X PUT -d '{"user_definition":"a greeting","personal_note":"learned before"}' "$base/v1/word-profiles/$profile_id/learning-content")"
json_assert "$details" 'v.profile.user_definition==="a greeting"&&v.profile.personal_note==="learned before"' "learning content should be persisted"

bundle="$(api_curl "$base/v1/vocabulary/export")"
json_assert "$bundle" 'v.version===3&&v.profiles.length===2' "export should have version 3 with 2 profiles"
