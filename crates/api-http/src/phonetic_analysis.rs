...
use domain::{
    DetectedPhone, PhoneticAnalysis, PhoneticAnalysisId, PhoneticAnalysisJob,
    PhoneticAnalysisJobId, PhoneticAnalysisJobStatus, PhoneticAnalysisModelDescriptor,
    PhoneticAnalysisModelId, PhoneticAnalysisProviderInfo, PhoneticAnalysisScope,
    PhoneticFindingFeedback, PhoneticFindingId, PhoneticModelState, SubtitleSentence,
    SubtitleSentenceId, SubtitleTokenKind, SubtitleTrackId,
};
        ...
        let analyses = if job.scope == PhoneticAnalysisScope::Track {
            self.services
                ...
                .map(|sentence| {
                    ...
                    fake_analysis(&sentence_job, Some(&sentence))
                })
                .collect::<Result<Vec<_>, _>>()?
        } else {
            let sentence = job
                ...
                .transpose()?
                .flatten();
            vec![fake_analysis(&job, sentence.as_ref())?]
        };
...
fn fake_analysis(
    ...
    if job_research_mode(job).as_deref() == Some("partial") && phones.len() > 1 {
        phones.pop();
    }
