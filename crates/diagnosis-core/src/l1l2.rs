...
fn collect_family_spans(frame: &RhythmFrame, family: &str, spans: &mut Vec<L1DifficultyEvidenceSpan>) {
    match family {
        FAMILY_WEAK_GROUP => {
...
mod tests {
    ...
    fn connected_speech_ref_needs_matching_hotspot_span() {
        ...
        assert_eq!((hits[0].spans[0].start_ms, hits[0].spans[0].end_ms), (800, 1200));
    }
