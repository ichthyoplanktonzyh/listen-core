...
- UX 导航更新：2026-07-15，四通道不建设四套割裂的 Studio 首页。产品采用内容会话与
  资产驱动两条旅程：同一真实内容内听/读/说/写是一跳通道切换，复习、狩猎、个人表达
  与 Coach 从资产层发起跨媒体任务；首页负责恢复内容现场与分发到期资产。
- 产品定义更新：2026-07-20，owner 全局旅程 QA 确立“明确内容选区”和“目标不变的降级”
  两条硬约束。学习任务必须显示并冻结有界输入；听/读可使用全文或选区，说/写默认使用一句
  或短段，不从当前播放位置推断无界全文。降级只有在保持同一用户目标与事实权威时成立；
  来源快照用于保留历史，不以文字冒充不可用的音频任务。见 ADR 0025、0026。
- 产品收口更新：2026-07-20 14:46 CST，Phase 3.19 的目标不是完成清单，而是让保留产品真正
  可用、好用。遗留 P1、入口、任务边界、结果可见性和返回路径未收口前禁止新功能；对 Phase
  3.x 全部用户功能与 fallback 执行 `KEEP / SIMPLIFY / REMOVE / UNAVAILABLE / DEFER RESEARCH`
  审计。已完成代码、未来可能有用或“提供一种降级”不能证明需求成立。
...
- **减法优先于功能沉积**：用户目标、稳定入口、可理解结果或真实使用证据缺失的功能不因代码
  已存在而保留；重复、预设性或只服务内部模型的 surface 应简化或删除。产品可用性债未清偿前
  不启动新功能建设。
...
- **任务输入必须有界且可见**：内容文档是浏览范围，内容选区才是一次学习任务的输入。
  当前播放位置只能帮助建立选区，不能隐式代表“从这里一直到全文结束”；任务开始后冻结
  选区文本与边界，避免播放移动或来源变化改写任务。
...
- **能力矩阵优先，降级必须保持目标**：每种语言声明支持哪些能力（分词、词典、发音、诊断、
  时间轴）。缺失能力应显示原因、恢复动作或保持同一目标的替代路径；若替代会改变媒介、任务
  或事实权威，则明确标为不可用，不得假装原任务已完成。
